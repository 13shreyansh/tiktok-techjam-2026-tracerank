#!/usr/bin/env python3
"""Run one isolated, auditable Track 2 Run-5 iteration."""
...
RUN_DIR = ROOT / "experiments" / "run5"
LEDGER = RUN_DIR / "ledger.jsonl"
STATE = RUN_DIR / "run_state.json"
OUTPUTS = ROOT / "outputs" / "experiments" / "run5"
...
def main() -> None:
    ...
    if state["iterations"] >= MAX_ITERATIONS:
        raise SystemExit("Run 5 has reached its 50-iteration limit")
    if elapsed_total >= MAX_SECONDS:
        raise SystemExit("Run 5 has reached its six-hour wall-clock limit")
    ...
    if "--evaluate-test" in model_args:
        raise SystemExit("Run 5 public-test labels are locked; --evaluate-test is forbidden")
    ...
    try:
        ...
        proc = types.SimpleNamespace(
            returncode=124,
            stdout=stdout,
            stderr=stderr + "\nterminated by Run-5 ten-minute timeout",
        )
